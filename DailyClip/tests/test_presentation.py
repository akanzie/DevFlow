...
def test_workspace_renders_browse_entries_when_query_is_empty(qapp):
    ...
    assert window._results_list.count() == 1
    assert "[folder]" in window._results_list.item(0).text()
...
def test_workspace_refresh_items_uses_current_mode(qapp):
    ...
    assert window._refresh_button.text() == "Refresh"
...
def test_workspace_auto_refresh_only_runs_for_visible_browser_tab(qapp):
    ...
    window.show()
    qapp.processEvents()
    window._tabs.setCurrentWidget(window._browser_tab)
    window._auto_refresh_visible_data()

    assert calls == [True]

    calls.clear()
    window._tabs.setCurrentWidget(window._note_tab)
    window._auto_refresh_visible_data()
...
def test_workspace_switches_to_image_preview_for_image_entries(qapp, tmp_path):
    ...
    assert window._preview_stack.currentIndex() == 1
